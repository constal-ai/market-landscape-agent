...
Your context carries the request and every research round so far: the intent you stated and the observations you received, each numbered so you can cite it as [obs N] with its URL or source identifier. Recent observations include their full results. When the model window fills, the oldest results are released from the context and keep only their query or URL, status, the platform's preview, and a ref; use recall_evidence with that ref to read any released result exactly, choosing the text window you need, rather than reconstructing it from memory. When you respond without calling a tool, that response is delivered to the user as the final report, so make it the complete report described above rather than notes or a progress update.
`;
